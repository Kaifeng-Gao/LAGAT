...
class LAGATConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # (LAGAT) The learnable label embedding
        ...
        # (LAGAT) The learnable parameters to compute attention coefficients for label
    ...
    def reset_parameters(self):
            ...
            glorot(self.att_lab) # (LAGAT) 
            glorot(self.label_embs) # (LAGAT) 
            zeros(self.bias)
    ...
    def forward(  # noqa: F811
        ...
        # (LAGAT) Compute label level attention coefficients
        ...
        # edge_updater_type: (alpha: OptPairTensor, edge_attr: OptTensor, alpha_label: Tensor, label_index: Tensor)
        alpha = self.edge_updater(
            edge_index, 
            size=size, 
            alpha=alpha, 
            edge_attr=edge_attr,
            alpha_label=alpha_lab, # (LAGAT)
            label_index=label_index # (LAGAT)
        )
    ...
    def edge_update(self, alpha_j: Tensor, alpha_i: OptTensor,
                    edge_index_j: Tensor, edge_index_i: OptTensor, 
                    edge_attr: OptTensor, index: Tensor, ptr: OptTensor,
                    alpha_label: Tensor, label_index: Tensor,
                    dim_size: Optional[int]) -> Tensor:
        # Given edge-level attention coefficients for source and target nodes,
        # we simply need to sum them up to "emulate" concatenation:
        alpha = alpha_j if alpha_i is None else alpha_j + alpha_i

        # (LAGAT) Label attention with mask
        loop_edge_mask = edge_index_i == edge_index_j
        label_j = torch.index_select(label_index, 0, edge_index_j)
        label_j = torch.where(loop_edge_mask, torch.zeros_like(label_j), label_j)
        alpha = alpha + torch.index_select(alpha_label, 0, label_j)
